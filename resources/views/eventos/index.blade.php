...
@section('titulo')
<title>Eventos - Inarte</title>
@stop

@section('cabecera')
@include('layouts.breadcrumb', ['titulo' => "Eventos", 'tituloModulo' => "Eventos"])
@stop
	...
	<table id="sample-table-2" class="table table-striped table-bordered table-hover">
		...
		<tbody>
            ...
            <tr>
				...
				<td>{{ $evento->lugar }}</td>
                <td class="hidden-480">
                    <a href="{{ URL::route('eventos.show', $evento->id) }}" data-rel="tooltip" title="Mostrar {{ $evento->id }}" objeto="{{ $evento->id }}" style="text-decoration:none;" data-id="{{ $evento->id }}"> 
                        <span class="btn btn-mini btn-info"> <i class="icon-eye-open bigger-120"></i> </span> 
                    </a>
                    &nbsp;
                    <a href="{{ URL::route('eventos.edit', $evento->id) }}" class="tooltip-success editar" data-rel="tooltip" title="Editar {{ $evento->id }}" objeto="{{ $evento->id }}" style="text-decoration:none;" data-id="{{ $evento->id }}"> 
